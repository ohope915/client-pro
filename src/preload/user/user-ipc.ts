 ...
 * 同步用户状态到主进程
 */
async function syncUserStateHandler(): Promise<void> {
	ipcMain.on("sync-user-state", async (_event, userState: UserState) => {
 ...
 * 清除用户状态
 */
async function clearUserStateHandler(): Promise<void> {
...
export const regUserIPC = () => {
	syncUserStateHandler()
	getUserAccountHandler()
	clearUserStateHandler()
